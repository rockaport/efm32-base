 ...
 * stack-related errors, such as CCA failures, after which the plugin gives up.
 */
 ...
 * the target devices.
 *
 * @param[in] targetListLength   The length of the passed target list.
 *
 ...
 * - ::EMBER_OTA_BROADCAST_BOOTLOADER_STATUS_NO_BUFFERS if the server currently
 * doesn't have enough RAM to store the passed target list.
 */
...
/** @brief  Initiates the process to request the status of a set
 * of target devices.
 * The application can use the ::emberAfPluginBootloaderServerGetTargetStatus()
...
/** @brief  Starts the process where a server requests a set of target
...
/** @brief  Retrieves the status of an individual target in the
 * distribution list.
 *
 ...
 * - ::EMBER_OTA_BROADCAST_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_ACCEPTED is
 ...
 * - ::EMBER_OTA_BROADCAST_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_REFUSED is
 * the server is currently performing a bootload request process and the target
 * has refused to perform the requested image bootload.
 * - ::EMBER_OTA_BROADCAST_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_ONGOING is
 * the server is currently performing a bootload request process and the target
...
/** @brief  Aborts the ongoing process (image distribution,
 ...
 * OTA Server plugin aborts the ongoing distribution process.
 */
