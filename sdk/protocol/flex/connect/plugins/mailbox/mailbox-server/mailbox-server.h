...
/** @brief Adds a message to the mailbox queue. The message is stored in the
 * internal queue until the destination node queries the mailbox server node for
 * messages or upon timeout.
 *
 *  @param[in] destination   An ::EmberNodeId value indicating the address of
 *  the message's destination node.
 *
 *  @param[in] message    A pointer to the message to be enqueued. Note that, if
 *  the allocation memory plugin option is enabled, the packet content is
 *  copied in the internal plugin data structures. Otherwise, the plugin
 *  stores the pointer. In this case, the application is responsible to
 ...
 *
 *  @param[in] tag    A tag value for the passed message.
 *
 *  @param[in] useSecurity   Indicates whether the message should be sent
 *  secured or not.
 *
 ...
 *
 * @param[in] messageDestination   The message destination.
 *
