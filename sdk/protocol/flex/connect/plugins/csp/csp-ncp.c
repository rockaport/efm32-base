...
// macGetParentAddress
static void ncpMacGetParentAddressCommand(void)
{
  EmberMacAddress parentAddress;
  MEMSET(&parentAddress, 0, sizeof(EmberMacAddress));
  parentAddress.addr.shortAddress = emberUnsignedCommandArgument(0);
  emberGetStringArgument(1, parentAddress.addr.longAddress, EUI64_SIZE, false);
  parentAddress.mode = emberUnsignedCommandArgument(2);
  EmberStatus status = emberMacGetParentAddress(&parentAddress);
  emSendBinaryManagementCommand(EMBER_MAC_GET_PARENT_ADDRESS_RETURN_IDENTIFIER,
                                "uvbu",
                                status,
...
static void ncpCalibrateCurrentChannelCommand(void)
{
  emberCalibrateCurrentChannel();
}

// getMaximumPayloadLength
static void ncpGetMaximumPayloadLengthCommand(void)
{
  EmberMacAddressMode srcAddressMode;
  EmberMacAddressMode dstAddressMode;
  bool interpan;
  bool secured;
  srcAddressMode = (EmberMacAddressMode) emberUnsignedCommandArgument(0);
  dstAddressMode = (EmberMacAddressMode) emberUnsignedCommandArgument(1);
  interpan = (bool) emberUnsignedCommandArgument(2);
  secured = (bool) emberUnsignedCommandArgument(3);
  uint8_t payloadLength = emberGetMaximumPayloadLength(srcAddressMode,
                                                       dstAddressMode,
                                                       interpan,
                                                       secured);
  emSendBinaryManagementCommand(EMBER_GET_MAXIMUM_PAYLOAD_LENGTH_RETURN_IDENTIFIER,
                                "u",
                                payloadLength);
}
...
{
  emSendBinaryManagementCommand(CB_RADIO_NEEDS_CALIBRATING_COMMAND_IDENTIFIER,
                                "");
}
...
// macMessageSend
static void ncpMacMessageSendCommand(void)
{
  ...
  uint8_t *message;
  EmberMessageOptions options;
  MEMSET(&macFrame, 0, sizeof(EmberMacFrame));
  macFrame.srcAddress.addr.shortAddress = emberUnsignedCommandArgument(0);
  emberGetStringArgument(1, macFrame.srcAddress.addr.longAddress, EUI64_SIZE, false);
  macFrame.srcAddress.mode = emberUnsignedCommandArgument(2);
  macFrame.dstAddress.addr.shortAddress = emberUnsignedCommandArgument(3);
  emberGetStringArgument(4, macFrame.dstAddress.addr.longAddress, EUI64_SIZE, false);
  macFrame.dstAddress.mode = emberUnsignedCommandArgument(5);
  macFrame.srcPanId = emberUnsignedCommandArgument(6);
  macFrame.dstPanId = emberUnsignedCommandArgument(7);
  macFrame.srcPanIdSpecified = emberUnsignedCommandArgument(8);
  macFrame.dstPanIdSpecified = emberUnsignedCommandArgument(9);
  messageTag = emberUnsignedCommandArgument(10);
  messageLength = emberUnsignedCommandArgument(11);
  message = (uint8_t *)emberStringCommandArgument(12, &(messageLength));
  options = emberUnsignedCommandArgument(13);
  EmberStatus status = emberMacMessageSend(&macFrame,
                                           messageTag,
                                           messageLength,
                                           message,
                                           options);
  emSendBinaryManagementCommand(EMBER_MAC_MESSAGE_SEND_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// macSetPanCoordinator
static void ncpMacSetPanCoordinatorCommand(void)
{
  bool isCoordinator;
  isCoordinator = (bool) emberUnsignedCommandArgument(0);
  EmberStatus status = emberMacSetPanCoordinator(isCoordinator);
  emSendBinaryManagementCommand(EMBER_MAC_SET_PAN_COORDINATOR_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// setPollDestinationAddress
static void ncpSetPollDestinationAddressCommand(void)
{
  EmberMacAddress destination;
  MEMSET(&destination, 0, sizeof(EmberMacAddress));
  destination.addr.shortAddress = emberUnsignedCommandArgument(0);
  emberGetStringArgument(1, destination.addr.longAddress, EUI64_SIZE, false);
  destination.mode = emberUnsignedCommandArgument(2);
  EmberStatus status = emberSetPollDestinationAddress(&destination);
  emSendBinaryManagementCommand(EMBER_SET_POLL_DESTINATION_ADDRESS_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// removeChild
static void ncpRemoveChildCommand(void)
{
  EmberMacAddress address;
  MEMSET(&address, 0, sizeof(EmberMacAddress));
  address.addr.shortAddress = emberUnsignedCommandArgument(0);
  emberGetStringArgument(1, address.addr.longAddress, EUI64_SIZE, false);
  address.mode = emberUnsignedCommandArgument(2);
  EmberStatus status = emberRemoveChild(&address);
  emSendBinaryManagementCommand(EMBER_REMOVE_CHILD_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// getChildFlags
static void ncpGetChildFlagsCommand(void)
{
  EmberMacAddress address;
  ...
  emberGetStringArgument(1, address.addr.longAddress, EUI64_SIZE, false);
  address.mode = emberUnsignedCommandArgument(2);
  EmberStatus status = emberGetChildFlags(&address,
                                          &flags);
  emSendBinaryManagementCommand(EMBER_GET_CHILD_FLAGS_RETURN_IDENTIFIER,
                                "uu",
                                status,
                                flags);
}
...
static void ncpPurgeIndirectMessagesCommand(void)
{
  EmberStatus status = emberPurgeIndirectMessages();
  emSendBinaryManagementCommand(EMBER_PURGE_INDIRECT_MESSAGES_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// macAddShortToLongAddressMapping
static void ncpMacAddShortToLongAddressMappingCommand(void)
{
  EmberNodeId shortId;
  uint8_t longId[EUI64_SIZE];
  shortId = (EmberNodeId) emberUnsignedCommandArgument(0);
  emberGetStringArgument(1, longId, EUI64_SIZE, false);
  EmberStatus status = emberMacAddShortToLongAddressMapping(shortId,
                                                            longId);
  emSendBinaryManagementCommand(EMBER_MAC_ADD_SHORT_TO_LONG_ADDRESS_MAPPING_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// macClearShortToLongAddressMappings
static void ncpMacClearShortToLongAddressMappingsCommand(void)
{
  EmberStatus status = emberMacClearShortToLongAddressMappings();
  emSendBinaryManagementCommand(EMBER_MAC_CLEAR_SHORT_TO_LONG_ADDRESS_MAPPINGS_RETURN_IDENTIFIER,
                                "u",
                                status);
}
...
// incomingMacMessageHandler
void emberIncomingMacMessageHandler(EmberIncomingMacMessage *message)
{
  emSendBinaryManagementCommand(CB_INCOMING_MAC_MESSAGE_COMMAND_IDENTIFIER,
                                "uvbuvbuvvuuuuwub",
                                message->options,
...
// macMessageSentHandler
void emberMacMessageSentHandler(EmberStatus status,
                                EmberOutgoingMacMessage *message)
{
  emSendBinaryManagementCommand(CB_MAC_MESSAGE_SENT_COMMAND_IDENTIFIER,
                                "uuvbuvbuvvuuuwubu",
                                status,
                                message->options,
                                message->macFrame.srcAddress.addr.shortAddress,
...
static void ncpSetApplicationBeaconPayloadCommand(void)
{
  ...
  uint8_t *payload;
  payload = (uint8_t *)emberStringCommandArgument(0, &(payloadLength));
...
static void ncpSetSelectiveJoinPayloadCommand(void)
{
  uint8_t payloadLength;
  uint8_t *payload;
  payload = (uint8_t *)emberStringCommandArgument(0, &(payloadLength));
  EmberStatus status = emberSetSelectiveJoinPayload(payloadLength,
                                                    payload);
  emSendBinaryManagementCommand(EMBER_SET_SELECTIVE_JOIN_PAYLOAD_RETURN_IDENTIFIER,
                                "u",
                                status);
}

// clearSelectiveJoinPayload
static void ncpClearSelectiveJoinPayloadCommand(void)
{
  EmberStatus status = emberClearSelectiveJoinPayload();
  emSendBinaryManagementCommand(EMBER_CLEAR_SELECTIVE_JOIN_PAYLOAD_RETURN_IDENTIFIER,
                                "u",
                                status);
}
...
// macFormNetwork
static void ncpMacFormNetworkCommand(void)
{
  EmberNetworkParameters parameters;
  MEMSET(&parameters, 0, sizeof(EmberNetworkParameters));
  parameters.panId = emberUnsignedCommandArgument(0);
  parameters.radioTxPower = emberUnsignedCommandArgument(1);
  parameters.radioChannel = emberUnsignedCommandArgument(2);
  EmberStatus status = emberMacFormNetwork(&parameters);
  emSendBinaryManagementCommand(EMBER_MAC_FORM_NETWORK_RETURN_IDENTIFIER,
                                "u",
                                status);
}
...
{
  emSendBinaryManagementCommand(CB_INCOMING_BEACON_EXTENDED_COMMAND_IDENTIFIER,
                                "vvbuuuubub",
                                panId,
...
const EmberCommandEntry emberCommandTable[] =
{
  ...
  emberBinaryCommandEntryAction(EMBER_GET_EUI64_COMMAND_IDENTIFIER,
                                ...
                                "",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_GET_PARENT_ADDRESS_COMMAND_IDENTIFIER,
                                ncpMacGetParentAddressCommand,
                                "vbu",
                                NULL),
  ...
  emberBinaryCommandEntryAction(EMBER_GET_CSP_VERSION_COMMAND_IDENTIFIER,
                                ...
                                "",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_CALIBRATE_CURRENT_CHANNEL_COMMAND_IDENTIFIER,
                                ncpCalibrateCurrentChannelCommand,
                                "",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_GET_MAXIMUM_PAYLOAD_LENGTH_COMMAND_IDENTIFIER,
                                ncpGetMaximumPayloadLengthCommand,
                                "uuuu",
                                NULL),
  ...
  emberBinaryCommandEntryAction(EMBER_POLL_FOR_DATA_COMMAND_IDENTIFIER,
                                ...
                                "",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_MESSAGE_SEND_COMMAND_IDENTIFIER,
                                ncpMacMessageSendCommand,
                                "vbuvbuvvuuuubu",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_SET_PAN_COORDINATOR_COMMAND_IDENTIFIER,
                                ncpMacSetPanCoordinatorCommand,
                                "u",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_SET_POLL_DESTINATION_ADDRESS_COMMAND_IDENTIFIER,
                                ncpSetPollDestinationAddressCommand,
                                "vbu",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_REMOVE_CHILD_COMMAND_IDENTIFIER,
                                ncpRemoveChildCommand,
                                "vbu",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_GET_CHILD_FLAGS_COMMAND_IDENTIFIER,
                                ncpGetChildFlagsCommand,
                                "vbu",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_PURGE_INDIRECT_MESSAGES_COMMAND_IDENTIFIER,
                                ncpPurgeIndirectMessagesCommand,
                                "",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_ADD_SHORT_TO_LONG_ADDRESS_MAPPING_COMMAND_IDENTIFIER,
                                ncpMacAddShortToLongAddressMappingCommand,
                                "vb",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_CLEAR_SHORT_TO_LONG_ADDRESS_MAPPINGS_COMMAND_IDENTIFIER,
                                ncpMacClearShortToLongAddressMappingsCommand,
                                "",
                                NULL),
  ...
  emberBinaryCommandEntryAction(EMBER_SET_APPLICATION_BEACON_PAYLOAD_COMMAND_IDENTIFIER,
                                ...
                                "b",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_SET_SELECTIVE_JOIN_PAYLOAD_COMMAND_IDENTIFIER,
                                ncpSetSelectiveJoinPayloadCommand,
                                "b",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_CLEAR_SELECTIVE_JOIN_PAYLOAD_COMMAND_IDENTIFIER,
                                ncpClearSelectiveJoinPayloadCommand,
                                "",
                                NULL),
  ...
  emberBinaryCommandEntryAction(EMBER_JOIN_NETWORK_COMMAND_IDENTIFIER,
                                ...
                                "uvuu",
                                NULL),
  emberBinaryCommandEntryAction(EMBER_MAC_FORM_NETWORK_COMMAND_IDENTIFIER,
                                ncpMacFormNetworkCommand,
                                "vuu",
                                NULL),
