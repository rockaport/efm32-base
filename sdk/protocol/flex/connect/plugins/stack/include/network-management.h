/***************************************************************************//**
 * @brief See @ref network_management for documentation.
 *******************************************************************************
 * # License
 ...
 *
 * @param[in] nodeId  The source node ID of the received beacon.
 *
 ...
 *
 * @param[in] payload  A pointer to the application beacon payload of the
 *  received beacon.
 */
 ...
 *
 * @param[in] beaconFields   A pointer to the the beacon fields defined as per
 ...
 *   This can not exceed ::EMBER_MAX_JOIN_PAYLOAD_LENGTH.
 *
...
/** @brief It clears the join payload previously set with the
 * ::emberSetJoinPayload() API. When invoked at a ::EMBER_STAR_COORDINATOR or a
 * ::EMBER_STAR_RANGE_EXTENDER it causes the stack to accept joining nodes with
 * any join payload pattern. When invoked at a node that has not yet joined a
 * network, it clears the join payload. Subsequent joining attempts will not
 * include any join payload in the over-the-air joining handshake.
 *
 * @return An ::EmberStatus value that indicates either:
...
#if defined(DOXYGEN_SHOULD_SKIP_THIS) || defined(UNIX_HOST)
/** @brief Forms a new network as a ::EMBER_MAC_MODE_DEVICE by becoming the
 *  coordinator. This API should be used to form a compliant 802.15.4 PAN and
 *  to interoperate with other 802.15.4 devices.
 *
 * @param[in] parameters  An ::EmberNetworkParameters value that specifies the
 * network parameters of the network to be formed.
 *
 ...
 * source node long ID. A receiver must do the same in order to decrypt a
 ...
 *
 * @note This table is stored in RAM, therefore the application should ensure
 ...
 * layer.
 *
 * @return an ::EmberStatus value of ::EMBER_SUCCESS if table was , or another
