 ...
 *
 * @param[in] message  An ::EmberMacIncomingMessage describing the incoming
 * packet.
 */
 ...
 *
 * @param[in] messageLength The size in bytes of the message payload. Use the
 ...
 * is 0, the passed TX options indicates some feature that is not supported or
 * the destination address mode is set to ::EMBER_MAC_ADDRESS_MODE_NONE.
 * - ::EMBER_MESSAGE_TOO_LONG if the message does not fit in a single frame.
 * - ::EMBER_PHY_TX_BUSY if the message cannot be sent since the node does not
 * support MAC queuing and the radio is currently busy.
 * - ::EMBER_MAC_TRANSMIT_QUEUE_FULL if the outgoing MAC queue is currently
 ...
 *
 * @param[in] intrapan  Indicates whether the frame is an interpan frame or not.
