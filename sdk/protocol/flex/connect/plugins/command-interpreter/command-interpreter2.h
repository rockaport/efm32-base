 ...
 * Interprets serial port commands. See command-interpreter2.c for source code.
 *
 ...
 *   -# An ::EmberCommandEntry::argumentTypes string specifies the number and
 *      types of arguments
 ...
 *    deal with the incorrect command input. Applications may override it.
 ...
 * within command functions.  If an error occurs in the command input,
 * ::emberCommandErrorHandler() is called instead of a command function.
 *
 ...
 * match, an error of type EMBER_CMD_ERR_NO_SUCH_COMMAND is generated.
...
/** Indicates whether or not the command entry structure includes descriptions
 *  for the commands.  This consumes additional CONST space, which is expensive
...
typedef PGM struct {
   ...
   *  command.
   *  If this entry refers to a nested command, the action field
   *  has to be set to NULL.
   */
  CommandAction action;

  /**
   * For normal, non-nested, commands, argumentTypes is a
   ...
   *
   *  For a nested command (action is NULL), argumentTypes will
 ...
 *
 * If you change this list, also change the strings that describe
 ...
 * Use the following functions that process commands to
 * retrieve arguments from the command interpreter.
 * These functions pull out unsigned integers, signed integers, strings,
 * and hexadecimal strings.  Index 0 is the first command argument.
 *@{
 */
...
/** Retrieves quoted string or hexadecimal string arguments.
 * Hexadecimal strings are already converted into binary.
 * ASCII strings are null terminated.  The null terminator
 * is not included in the returned length argument.
 * To retrieve the name of the command itself, use an argNum of -1.
 * For example, to retrieve the first character of the command,
 * uint8_t firstChar = emberStringCommandArgument(-1, NULL)[0].
 * If the command is nested, an index of -2, -3, and so on retrieves
...
// with the command line name, however the caller must use the length to print
// it; It's not NULL-terminated, unless it happens to be the only text entered
// on the command line.
// emberCommandName() returns a NULL-terminated command name, which is easier
// than requiring every caller to know the command name length.
 ...
 * If the argument length is not zero but less than maxLength
...
/** @brief Processes the given string as a command.
 */
bool emberProcessCommandString(const uint8_t *input, uint16_t sizeOrPort);

/** @brief Processes the input coming in on the given serial port.
...
/** @brief Returns the value of a hexadecimal digit ch (0 - 15).
 * Returns a value > 15 if ch is not a hexadecimal digit.
 */
...
typedef struct {
  ...
  // Storage for reading in a hexadecimal string. A value of 0xFF means unused.
  uint8_t hexHighNibble;

  // The token number of the first true argument after possible nested commands.
  uint8_t argOffset;

  // Remembers the previous character seen by emberProcessCommandString()
  ...
  // Keeps a pointer to the current command the user ran.
...
/** @brief For use when additional different command streams are used.
 */
