...
void halInternalSetMfgTokenData(uint16_t token, void *data, uint32_t len)
{
  ...
  if ((token & 0xF000) == (USERDATA_TOKENS & 0xF000)) {
    ...
    // write first word if necessary
    if (isStartWord) {
      flashStatus = halInternalFlashWriteSeries2(startWordAddress, &startWordToWrite, 1);
    ...
    // write last word if necessary
    if (isEndWord) {
      flashStatus = halInternalFlashWriteSeries2(endWordAddress, &endWordToWrite, 1);
      assert(flashStatus == EMBER_SUCCESS);
    }
  }
...
#ifndef EMBER_TEST
uint16_t getMfgTokenAddress(uint16_t creator)
{
...
uint8_t getMfgTokenSize(uint16_t creator)
{
  uint8_t tokenSize;
...
uint8_t getMfgTokenArraySize(uint16_t creator)
{
